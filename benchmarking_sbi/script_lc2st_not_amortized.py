...
if __name__ == "__main__":
    ...
    inv_flow_samples_ref = torch.load(PATH_EXPERIMENT / "inv_flow_samples_ref.pkl")
    ...
    if args.z_space:
        ...
        path_probas_null = (
            PATH_EXPERIMENT / f"lc2st_probas_null_z_False_{args.clf_name}.pkl"
        )
    ...
    if args.run_htest:
        ...
        if args.z_space:
            ...
            if os.path.exists(PATH_EXPERIMENT / f"P_eval_z_{args.z_space}.pkl"):
    ...
    probas_ens = torch.load(
        PATH_EXPERIMENT / f"lc2st_probas_ensemble_z_{args.z_space}_{args.clf_name}.pkl"
    )

    probas_null = torch.load(path_probas_null)

    lc2st_htest_results = torch.load(
        PATH_EXPERIMENT / f"lc2st_htest_results_z_{args.z_space}_{args.clf_name}.pkl"
    )
    ...
    plt.savefig(PATH_EXPERIMENT / f"lc2st_pp_plot_z_{args.z_space}_{args.clf_name}.pdf")
    plt.show()
    ...
    if args.correct_posterior:
        ...
        if args.z_space:
            ...
            plt.savefig(
                PATH_EXPERIMENT / f"z_space_correction_corner_{args.clf_name}.pdf"
            )
            plt.show()
