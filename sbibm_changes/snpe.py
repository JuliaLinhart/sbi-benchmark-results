...
def run(
    ...
    ## ==== change: start - sampling as forward transformation ====
    # samples = posterior.sample((num_samples,)).detach()

    # this is not the same as transforming...
    # need it for comparison purposes in posterior correction...

    samples_z = posterior.flow.posterior_estimator._distribution.sample(
        num_samples
    ).detach()
    observation_emb = posterior.flow.posterior_estimator._embedding_net(observation)
    z_repeated, x_repeated = match_theta_and_x_batch_shapes(samples_z, observation_emb)
    samples = posterior.flow.posterior_estimator._transform.inverse(
        z_repeated, x_repeated
    )[0].detach()
    ...
    if num_observation is not None:
        ...
        ## ==== change: start - add posterior as output ====
